...
function PopularCategoriesSection() {
  ...
  return (
    <div className="w-full pt-[clamp(16px,2vw,24px)] pb-[clamp(8px,1vw,12px)] px-4 bg-white block min-[1064px]:hidden">
      ...
      <div className="flex gap-4 md:gap-6 overflow-x-auto overflow-y-hidden pb-[clamp(8px,1vw,12px)] pt-2 pl-2 pr-4 md:pr-6 [-webkit-overflow-scrolling:touch] [&::-webkit-scrollbar]:hidden [&::-webkit-scrollbar]:w-0 [&::-webkit-scrollbar]:h-0 [-ms-overflow-style:none] [scrollbar-width:none]" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {popularCategories.map((category, index) => (
          <div 
            key={index} 
            className="bg-white rounded-[clamp(28px,4vw,40px)] shadow-[0_-2px_6px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.08),-2px_0_6px_rgba(0,0,0,0.08),2px_0_6px_rgba(0,0,0,0.08)] overflow-visible cursor-pointer flex-shrink-0 w-[calc((100%-16px)/2)] min-w-[calc((100%-16px)/2)] max-w-[calc((100%-16px)/2)] md:w-[calc((100%-24px)/2)] md:min-w-[calc((100%-24px)/2)] md:max-w-[calc((100%-24px)/2)] min-[1063px]:w-[calc((100%-24px)/2)] min-[1063px]:min-w-[calc((100%-24px)/2)] min-[1063px]:max-w-[calc((100%-24px)/2)] aspect-[3/4] transition-all flex flex-col p-[clamp(12px,1.5vw,16px)] box-border relative hover:-translate-y-1"
