...
function PopularCategoriesSection() {
  ...
  return (
    <div className="w-full pt-[clamp(16px,2vw,24px)] pb-[clamp(8px,1vw,12px)] px-4 bg-white">
      ...
      <div className="flex gap-3 md:gap-5 overflow-x-auto overflow-y-hidden pb-[clamp(8px,1vw,12px)] pt-2 pl-2 scrollbar-thin scrollbar-thumb-[#cbd5e0] scrollbar-track-transparent [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#cbd5e0] [&::-webkit-scrollbar-thumb]:rounded-sm [-webkit-overflow-scrolling:touch]">
        {popularCategories.map((category, index) => (
          <div 
            key={index} 
            className="bg-white rounded-[clamp(28px,4vw,40px)] shadow-[0_-2px_6px_rgba(0,0,0,0.08),0_2px_6px_rgba(0,0,0,0.08),-2px_0_6px_rgba(0,0,0,0.08),2px_0_6px_rgba(0,0,0,0.08)] overflow-visible cursor-pointer flex-shrink-0 w-[calc((100%-12px)/2)] min-w-[calc((100%-12px)/2)] max-w-[calc((100%-12px)/2)] md:w-[calc((100%-40px)/3)] md:min-w-[calc((100%-40px)/3)] md:max-w-[calc((100%-40px)/3)] lg:w-[calc((100%-60px)/4)] lg:min-w-[calc((100%-60px)/4)] lg:max-w-[calc((100%-60px)/4)] aspect-[3/4] transition-all flex flex-col p-[clamp(12px,1.5vw,16px)] box-border relative hover:-translate-y-1"
